...
export abstract class BaseMonitor extends EventEmitter {
  ...
  async init(): Promise<void> {
    ...
    this.accountCycle = new Cycle(
      shuffleArray(
        accounts
          .filter((account) => typeof account.access_token === "string")
          .map((account: Account) => {
  ...
  protected abstract process(result: ResponseResult): Promise<void>;
  ...
  protected getMapFN(
    ...
    processFN: (result: ResponseResult) => Promise<any>
    ...
    return async (product: InternalProduct) => {
      try {
        ...
        log(`[${product.asin}] ${err.toString()}`, "red");
  ...
  protected async startMonitorLoop(): Promise<void> {
    ...
    while (this.monitorStatus) {
      ...
      await Promise.allSettled(
        this.products.map(this.getMapFN(this.request, this.process))
      );
      log(
        `[${this.site}] Processed ${
